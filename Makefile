...
# << [CTFN-CMN-003] asdrick : for CCACHE mode
CROSS_CCACHE_MODE=n
export CROSS_CCACHE_MODE
ifneq ($(strip $(CROSS_CCACHE_MODE)),y)
CCACHE_DISABLE=1
export CCACHE_DISABLE
else
UNSET_CCACHE_ENV='sh -c unset CCACHE_DISABLE'
#CCACHE_DISABLE=0
#export CCACHE_DISABLE
...
all_postcheck1: profile_saved_check sanity_check ct_autoconf_clean ct_autoconf \
...
menuconfig:
	...
	$(CONFIG_SHELL) $(HOSTTOOLS_DIR)/scripts/CT_Menuconfig $(TARGETS_DIR)/config.in $(PROFILE)
...
ifeq ($(strip $(BRCM_KERNEL_ROOTFS)),jffs2)
	...
	$(HOSTTOOLS_DIR)/createimg --boardid=$(BRCM_BOARD_ID) --voiceboardid $(BRCM_VOICE_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --wholeflashfile=$(FLASH_NAND_CFEROM_FS_IMAGE_NAME_16).w; \
	...
	$(HOSTTOOLS_DIR)/createimg --boardid=$(BRCM_BOARD_ID) --voiceboardid $(BRCM_VOICE_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --wholeflashfile=$(FLASH_NAND_CFEROM_FS_IMAGE_NAME_128).w; \
...
# << [CTFN-CMN-006-1] Jimmy Wu : for NAND flash, 2012/08/08
ifneq ($(strip $(COMTREND_RELEASE_VERSION)),"")
ifneq ($(strip $(CTCONFIG_PRODUCT_MODEL)),"")
	@mkdir -p $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)
	@cp $(PROFILE_DIR)/$(FLASH_NAND_FS_IMAGE_NAME_16).w $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_NAND_IMAGE_16)
	@cp $(PROFILE_DIR)/$(FLASH_NAND_CFEROM_FS_IMAGE_NAME_16).w $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_NAND_CFE_IMAGE_16)
	@cp $(PROFILE_DIR)/$(FLASH_NAND_FS_IMAGE_NAME_128).w $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_NAND_IMAGE_128)
	@cp $(PROFILE_DIR)/$(FLASH_NAND_CFEROM_FS_IMAGE_NAME_128).w $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_NAND_CFE_IMAGE_128)
	@echo -e "Done! Release Files $(PROFILE) have been put in $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)."
endif
endif
# >> [CTFN-CMN-006-1] End
...
# << [CTFN-CMN-006] Jimmy Wu : for release use, 2008/08/20
ifneq ($(strip $(COMTREND_RELEASE_VERSION)),"")
ifneq ($(strip $(CTCONFIG_PRODUCT_MODEL)),"")
	@mkdir -p $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)
	@cp $(PROFILE_DIR)/$(FS_KERNEL_IMAGE_NAME) $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_IMAGE)
	@cp $(PROFILE_DIR)/$(CFE_FS_KERNEL_IMAGE_NAME) $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_CFE_IMAGE)
	@cp $(PROFILE_DIR)/$(FLASH_IMAGE_NAME) $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_FLASH)
	@cp $(PROFILE_DIR)/$(FLASH_IMAGE_NAME).w $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)/$(COMTREND_RELEASE_FLASH_W)
	@echo -e "Done! Release Files $(PROFILE) have been put in $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)."
endif
endif
# >> [CTFN-CMN-006] End
...
clean: target_clean bcmdrivers_clean data-model_clean userspace_clean kernel_clean \
       hosttools_clean voice_clean xchange_clean wlan_clean ct_autoconf_clean
	rm -f $(HOSTTOOLS_DIR)/scripts/lxdialog/*.o
	rm -f .tmpconfig*
	rm -f $(LAST_PROFILE_COOKIE)
	@# << [CTFN-CMN-005] Jimmy Wu : remove all images, rom and binary files
	rm -f images/*
	rm -f $(PROFILE_DIR)/*.rom
	rm -f $(PROFILE_DIR)/*.bin
	rm -f $(PROFILE_DIR)/*flash_image*
	rm -f $(PROFILE_DIR)/*.old
	rm -f $(PROFILE_DIR)/.h
	@# >> [CTFN-CMN-005] End
...
ifneq ($(strip $(COMTREND_RELEASE_VERSION)),"")
ifneq ($(strip $(CTCONFIG_PRODUCT_MODEL)),"")
	rm -fr $(PROFILE_DIR)/$(COMTREND_RELEASE_DIR)
endif
endif
	@# >> [CTFN-CMN-006] End
