...
#define bcmPktDma_EthXmit(p1,p2,p3,p4,p5,p6,p7,unused1,unused2,unused3) \
    bcmPktDma_EthXmitNoCheck_Iudma (p1,p2,p3,p4,p5,p6,p7)
...
#define bcmPktDma_EthSetPhyRate(_port, _enable, _kbps)
                                            ...
                                            uint32 noGlobalBufAccount)
{
    ...
    *pRxChannel = 0;
    ...
    // FIXME - txStateRam is only supported by those chip using XTMRT_IMPL=4
    else {
        txdma->txBds[bdIndex].status &=  ~DMA_OWN ; /* Clear the DMA_OWN flag, as this is force free */
...
//        fap4kePrt_Assert( ((uint32)txBd->address & 0xF) == 0 );
...
void bcmPktDma_EthSetPhyRate_Dqm(uint8 port, uint8 enable, int kbps);
