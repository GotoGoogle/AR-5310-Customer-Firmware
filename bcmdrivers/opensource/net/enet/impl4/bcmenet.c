...
extern unsigned int (*get_eth_state)(void);
//>> [CTFN-SYS-016] End
...
/*
 * Generic four ports:
 * "96368M-1331N", "96368M-123", "96368MT-1341N1"
 * "96368M-1341N", "96328A-134", "96328A-1341N"
 * "96328A-1241N", "96328A-1341N1"
 *
 * Generic five ports
 * "96368M-1331N",  "96368V-1331N", "96368M-1331N1"
 * "96368MT-1341N", "96368V-1341N"
 *
 * default:
 * 4 internal phys
 *     phy(0x1) = eth0
 ...
 *     phy(0x3) = eth2
 *     phy(0x4) = eth3
 * 4 internal phys + 1 external phy
 *     external phy = eth0
 *     phy(0x1) = eth1
 *     phy(0x2) = eth2
 *     phy(0x3) = eth3
 *     phy(0x4) = eth4
 */
int BpGetPhyOfEthMap( unsigned long *pulValue ) ;
static void devname_by_phyid(char* name, int phyid){
	static int initialized = 0, nGet = 0;
	static unsigned char PhyOfEthMap[BP_MAX_SWITCH_PORTS] = {0};
	char buf[IFNAMSIZ];
	int i;
	//<< [CTFN-DRV-001-2] Support to configure the first index of Ethernet interface in board parameters
	unsigned short EthIndex = 0;
	//>> [CTFN-DRV-001-2] End
	
//<< [CTFN-DRV-001-5] kewei lai : Avoid the phy ID error ,if define flag CONNECTED_TO_EXTERN_SW in boardparms.	
	/*remove flag CONNECTED_TO_EXTERN_SW,it will be use in 412L08 external switch*/
		phyid &=0xff; 
//>> [CTFN-DRV-001-5] End

	if(!initialized){
		initialized = 1;
		nGet = BpGetCtEthPhyMap(PhyOfEthMap);
	}
	if(nGet){
		if(vport_cnt != nGet){ 
			printk("\n\n[Comtrend] Warning: the maping between Ethx and PhyId is not well-defined in boardparms.c.\n%s:%s:%d\tvport_cnt=%d, nGet=%d\n\n", __FILE__, __FUNCTION__, __LINE__, vport_cnt, nGet);
		}else{
			//<< [CTFN-DRV-001-2] Support to configure the first index of Ethernet interface in board parameters
			if(BpGetCtEthIndex(&EthIndex) == BP_SUCCESS){
				printk("%s: change Eth index to %d \n", __FUNCTION__, EthIndex);
			}
			//<< [CTFN-DRV-001-4] Fix undefined ethindex needs to start with 0
			else {
				printk("%s: get Eth index fail \n", __FUNCTION__);
				EthIndex = 0;
			}
			//>> [CTFN-DRV-001-4] End
			//>> [CTFN-DRV-001-2] End
			for(i=0; i < BP_MAX_SWITCH_PORTS; i++){
				if(PhyOfEthMap[i]==(unsigned char)phyid){
					//<< [CTFN-DRV-001-2] Support to configure the first index of Ethernet interface in board parameters
					snprintf(buf, IFNAMSIZ, name, i + EthIndex);
					//>> [CTFN-DRV-001-2] End
					strlcpy(name, buf, IFNAMSIZ);
				}
			}
		}
	}else{ // default
		if(!IsExtPhyId(phyid)){
			if(vport_cnt == 4) snprintf(buf, IFNAMSIZ, name, (phyid -  1));
			else if(vport_cnt == 5)	snprintf(buf, IFNAMSIZ, name, phyid);
			strlcpy(name, buf, IFNAMSIZ);
		}
	}
}
...
static int create_vport(void)
{
    ...
    for (i = 1, j = 0; i < vport_cnt + 1; i++, j++, map /= 2)
    {
        ...
        {
//<< [CTFN-DRV-001] Antony.Wu : Support 1. Phyid to Ethx mapping and 2. IP101_1001 of ICPLUS, 20111205
            devname_by_phyid(dev->name, phy_id);
//>> [CTFN-DRV-001] End
...
//<< [CTFN-DRV-001] Antony.Wu : Support 1. Phyid to Ethx mapping and 2. IP101_1001 of ICPLUS, 20111205
#ifdef SUPPORT_IP101_IP1001
	if(pVnetDev0->unit == 0 && IsIP1001(findID(phy_id))){
		phys.lnk = 1;
		phys.spd1000 = 1;
    ...
    if ((priv->linkState & mask) != linkMask) {
        ...
        if (linkstatus) {
            ...
            if (speed == 1000)
            {
                ...
                bcmPktDma_EthSetPhyRate(port, 0, 990000);
            }
            else if (speed == 100)
            {
                mib->ulIfSpeed = SPEED_100MBIT;
                bcmPktDma_EthSetPhyRate(port, 1, 99000);
            }
            else
            {
                mib->ulIfSpeed = SPEED_10MBIT;
                bcmPktDma_EthSetPhyRate(port, 1, 9900);
            }
...
#endif
//<< Support to enable/disable EEE for Giga Ethernet ports to avoid IOP issue with some NICs (CSP #582132)
            ethsw_eee_port_enable(sw_port, 0, 0);
//>> Support to enable/disable EEE for Giga Ethernet ports to avoid IOP issue with some NICs (CSP #582132) End
...
//<< Wilber: add for NL-3110u GPHY LED, 2011/9/1
#if defined(CONFIG_BCM963268)
		if ( GPHY_PORT_ID == sw_port){
			printk("#######ifsetGPHYLED=%d, off = %llx\n",ifsetGPHYLED, LED_MODE_OFF);
			if ( ifsetGPHYLED ){
					SetGPHYSPDX(GPHY_SPD1, LED_MODE_OFF);
					ifsetGPHYLED = 0;
					}
			}
#endif
//>> End
            ...
            bcmPktDma_EthSetPhyRate(port, 0, 0);
...
//<< Wilber: add for NL-3110u GPHY LED, 2011/9/1
#if defined(CONFIG_BCM963268)
			if ( GPHY_PORT_ID == vport_to_phyport[i]){
				if ( speed == 10 ){
						SetGPHYSPDX(GPHY_SPD1, LED_MODE_ON);
						ifsetGPHYLED = 1;
					}
			}
#endif
...
//<< Fix ping failure to WAN with data length from 1465~1472 on Broadcom FAP based platform (CSP #533692), 2012/06/05
...
/*
 * Note: for IP101/1001 detection
 *   ethsw_reset_ports is modified to update sw.port_map, so I move the following function below ethsw_reset_ports.
 */
//>> [CTFN-DRV-001] End
#if defined(CONFIG_BCM963268)
...
//<< [CTFN-SYS-016] ChyanLong : Support Internet LED (implemented by Comtrend), 2009/02/12
unsigned int get_link_state(void)
{
#if 0
        /* Verify that no interface exists with this name */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,29)
...
#endif
	 unsigned int mask = 0x00000001 << port_id_from_dev(dev);
	 BcmEnet_devctrl *pDevCtrl = netdev_priv(vnet_dev[0]);
	 dev_put(dev);

     return pDevCtrl->linkState & mask ? 1 : 0;
#endif
	static BcmEnet_devctrl *pDevCtrl;
	pDevCtrl = netdev_priv(vnet_dev[0]);
       return  ((pDevCtrl->linkState & pDevCtrl->wanPort) != 0x00) ? 1 :0;
}
//>> [CTFN-SYS-016] End
