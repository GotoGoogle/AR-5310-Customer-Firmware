...
static int __init bcm63268_hw_init(void)
{
    ...
    int diff; 
#endif
 //<< kewei lai for fix wap-5892u Gphy Led
...
// << kewei lai for add new boardid 963169R-1861AC to WAP-5892u 
	//<< kewei lai for fix wap-5892u Gphy Led
	if(BpGetBoardId(boardid) == 0 && (strcmp(boardid, "963169R-1861AC") == 0 ))
	 {
	 
	 }
	else
	 {
    /*for eth wan 100 speed  connect to wps 2.4G led*/
...
static int __init bcm6318_hw_init(void)
{
    ...
    USBH->Setup |= USBH_IOC;
    USBH->Setup &= ~USBH_IPP;
    USBH->USBSimControl |= (1<<5); /*choose A1 implemenatation mode for Last ADDR calculation*/
